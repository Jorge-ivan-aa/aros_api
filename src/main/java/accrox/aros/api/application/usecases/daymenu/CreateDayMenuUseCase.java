...
import accrox.aros.api.application.exceptions.daymenu.DayMenuAlreadyExistsException;
import accrox.aros.api.application.exceptions.daymenu.InvalidDayMenuDateException;
...
public class CreateDayMenuUseCase {
    ...
    public void execute(CreateDayMenuInput input) throws
        ...
        DayMenuAlreadyExistsException,
        InvalidDayMenuDateException
    {
        LocalDate today = LocalDate.now();

        if (!today.equals(input.date())) {
            throw new InvalidDayMenuDateException();
        }

        if (this.repository.existsByDate(today)) {
            throw new DayMenuAlreadyExistsException();
        }
